...
if ($f == "admin_setting" && (auth()->admin == '1' || CheckUserPermission(auth()->id, $p))) {
        ...
        if (!empty($_GET['receipt_id'])) {
            ...
            if($receipt){
                ...
                if ($receipt['mode'] == 'credits') {
                    ...
                    if ($realprice == (int)$wo['config']['week_pro_plan']) {
                        $membershipType = 1;
                    } else if ($realprice == (int)$wo['config']['basic_pro_plan']) {
                        $membershipType = 2;
                    } else if ($realprice == (int)$wo['config']['premium_pro_plan']) {
                        $membershipType = 3;
                    } else if ($realprice == (int)$wo['config']['vip_pro_plan']) {
                        $membershipType = 4;
                    } else if ($realprice == (int)$wo['config']['vipgold_pro_plan']) {
                        $membershipType = 5;
                    } else if ($realprice == (int)$wo['config']['diamond_pro_plan']) {
                        $membershipType = 6;
                    }
                }
                ...
                if ($updated === true) {
                    ...
                    $query_one = mysqli_query($conn, "INSERT `payments`(`user_id`,`amount`,`type`,`pro_plan`,`credit_amount`,`via`, `status`,`checked`, `message`) VALUES ('{$receipt['user_id']}','{$receipt['price']}','PRO','{$membershipType}','{$amount}','Bank transfer', '1', '1', 'Bank transfer received');");
    ...
    if ($s == 'set_referrer' && isset($_GET['user_id']) && Wo_CheckSession($hash_id) === true) {
        $referrer = Secure($_GET['referrer']);
        $user_id = Secure($_GET['user_id']);
        $query = mysqli_query($conn, "UPDATE `users` SET `referrer` = '{$referrer}' WHERE `id` = '{$user_id}'");
        if ($query) {
            $data['status'] = 200;
            $data['status'] = 200;
    ...
    if ($s == 'fake-users') {
        ...
        $countries = array('IT' => 'Italy', 'DE' => 'Germany', 'CZ' => 'Czech Republic');
    ...
    if ($s == 'approve_user') {
        if (!empty($_GET['user_id'])) {
            ...
            if($receipt){
                $updated = $db->where('id',$_id)->update('users',array('verified'=>"1",'status'=>"1",'active'=>"1",'approved_at'=>time()));
                if ($updated === true) {
                    $user = $db->where('id', $_id)->getOne('users');

                    $message_body = Emails::parse('auth/activated', array(
                        'username' => $user['username'],

                    ));
                    $send         = SendEmail($user['email'], 'Account activated!', $message_body);
