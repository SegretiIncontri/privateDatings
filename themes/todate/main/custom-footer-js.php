...
<script>
            ...
            $.post(window.ajax + 'stripe/handle', {
                ...
                if (data.status == 200) {
                    ...
                    stripe.redirectToCheckout({
                        lineItems: [{
                            price: data.price_id.toString(), // Replace with the ID of your price
                            quantity: 1,
                        }],
                        clientReferenceId: data.paymentId.toString(),
                        mode: 'payment',
                        successUrl:data.location,
                        cancelUrl: DOMAIN + "/credits",
                        customerEmail: '<?php echo $profile->email;?>',
    ...
    function SH_StripeRequestPro(argument) {
            ...
            script.onload = script.onreadystatechange = function() {
                ...
                var stripe = Stripe('<?php echo $config->stripe_id;?>');
                $('#stripe_button_pro').attr('disabled', 'true');
                $('#stripe_button_pro').html("<span><?php echo __('Please wait..');?></span>");
    ...
    function SH_StripeRequestProUpdate(argument) {
        var script = document.createElement('script');
        var DOMAIN = window.location.origin;
        script.src = 'https://js.stripe.com/v3/';
        script.type = 'text/javascript';
        script.onload = script.onreadystatechange = function() {
            ...
            var checkoutButton = document.getElementById("stripe_button_pro");
            checkoutButton.addEventListener("click", function () {
                fetch("/ah/stripe/createSession", {
                    method: "POST",
                })
