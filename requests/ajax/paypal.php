        ...
        if (isset($_REQUEST[ 'paymentId' ]) && isset($_REQUEST[ 'PayerID' ])) {
            ...
            if (!is_array(PayPalCheckPayment($paymentId, $PayerID))) {
                if ($mode == 'credits') {
                    ...
                    if ($price == self::Config()->week_pro_plan) {
                        $membershipType = 1;
                    } else if ($price == self::Config()->basic_pro_plan) {
                        $membershipType = 2;
                    } else if ($price == self::Config()->premium_pro_plan) {
                        $membershipType = 3;
                    } else if ($price == self::Config()->vip_pro_plan) {
                        $membershipType = 4;
                    } else if ($price == self::Config()->vipgold_pro_plan) {
                        $membershipType = 5;
                    } else if ($price == self::Config()->diamond_pro_plan) {
                        $membershipType = 6;
                    }
